...
- Upstream base contained in current fork `main`: `832f0dfdd`
...
GitHub still records PR #48 as merged into `main` at `02b2dd52a44284aed273a19b8b5af35ab0f311d1`, but the current fork `main` now points at `380fb7fd016994dd13c1ed96aa1fc46159efbd22`, an upstream-aligned sync commit. Those two commits are not ancestors of each other.
...
- `node --test .github/scripts/console-live-promote-failure-issue.test.cjs`: passed, 18/18 tests.
- `node --check web/harness/scripts/compareBrowserMatrix.cjs`: passed.
- YAML parse with `js-yaml`: passed for `post-merge-verify.yml`, `console-live-promote.yml`, `console-live-macos-canary.yml`, and `console-live-promote-failure-issue.yml`.
