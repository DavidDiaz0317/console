...
import {
  ...
  buildMatrix,
  buildPulse,
} from "./github-pipelines-workflows";
...
export default async (req: Request): Promise<Response> => {
  ...
  try {
    if (view === "mutate") {
      ...
      for (const [key, value] of Object.entries(baseHeaders)) resp.headers.set(key, value);
      return resp;
    }

    const datePrefix = view === "pulse" ? new Date().toISOString().slice(0, 13) : "";
    ...
    switch (view) {
      ...
      case "log": {
        ...
        const response = await buildLog(token, repo, job);
        for (const [key, value] of Object.entries(baseHeaders)) response.headers.set(key, value);
        return response;
      }
      default:
    ...
    await writeCache(store, cacheKey, wrapped).catch((err) => {
      console.warn(
        "[github-pipelines] blob cache write failed:",
