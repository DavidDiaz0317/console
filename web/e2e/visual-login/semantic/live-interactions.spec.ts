  ...
  try {
    ...
    await test.step('user menu opens', async () => {
      await clickRequiredControl(page, '/', 'user menu', [
        page.locator('button').filter({ hasText: /DavidDiaz0317|live-canary-ui/i }).first(),
        page.getByRole('button', { name: /DavidDiaz0317|live-canary-ui|account|user/i }).first(),
        page.locator('button[aria-label*="user" i], button[aria-label*="account" i]').first(),
      ])
      await expect(page.locator('body'), 'user menu should expose account actions').toContainText(/profile|settings|sign out|logout|DavidDiaz0317|live-canary-ui/i)
      await page.keyboard.press('Escape').catch(() => undefined)
    })
