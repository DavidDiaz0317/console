...
/**
 * Safe setTimeout wrapper for the dynamic card sandbox.
 * Delegates to the real setTimeout but is provided explicitly so the
 * sandbox blocklist doesn't shadow it with undefined.
 */
...
/**
 * Safe clearTimeout wrapper for the dynamic card sandbox.
 */
...
/**
 * Safe setInterval wrapper for the dynamic card sandbox.
 * Enforces a minimum interval of MIN_INTERVAL_MS to prevent tight loops.
 */
...
/**
 * Safe clearInterval wrapper for the dynamic card sandbox.
 */
...
export function getDynamicScope(): Record<string, unknown> {
  return {
    ...
    // Safe timer APIs (clamped to MIN_INTERVAL_MS minimum for setInterval)
