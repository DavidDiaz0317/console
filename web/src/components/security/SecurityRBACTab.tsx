import { AlertTriangle, Key, Lock, ShieldCheck, ShieldX, Users } from 'lucide-react'
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  ...
                  <span className="text-xs px-2 py-0.5 rounded bg-card text-muted-foreground">{binding.kind}</span>
                </div>
                <div className="text-sm text-foreground mb-2">
                  ...
                  {binding.subjects.map((subject, j) => (
                    <span key={j} className="inline-flex items-center gap-1 mr-2">
                      {subject.kind === 'User' && <Users className="w-3 h-3" />}
                      {subject.kind === 'Group' && <Users className="w-3 h-3" />}
                      {subject.kind === 'ServiceAccount' && <Key className="w-3 h-3" />}
                      {subject.name}
                    </span>
                  ))}
                </div>
                <div className="flex flex-wrap gap-1">
                  {binding.permissions.slice(0, 5).map((permission, j) => (
