...
const LOAD_ERROR_MESSAGE = 'Failed to load insight preferences. Your browser storage may be unavailable.'
...
export function useInsightActions() {
  ...
  // Load persisted state after mount so we can surface errors via toast.
  useEffect(() => {
    setAcknowledgedIds(loadSet(localStorage, INSIGHT_ACKNOWLEDGE_KEY, (msg) => showToast(msg, 'error')))
    setDismissedIds(loadSet(sessionStorage, INSIGHT_DISMISS_KEY, (msg) => showToast(msg, 'error')))
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []) // run once on mount — showToast identity is stable
