...
export function ComplianceScore({ config: _config }: CardConfig) {
  ...
  const { statuses: kubescapeStatuses, aggregated: kubescapeAggregated, isLoading: kubescapeLoading, isRefreshing: kubescapeRefreshing, isDemoData: kubescapeDemoData, installed: kubescapeInstalled, hasErrors: kubescapeHasErrors, clustersChecked: kubescapeChecked, totalClusters: kubescapeTotal, unavailableReason: kubescapeUnavailable } = useKubescape()
  const { statuses: kyvernoStatuses, isLoading: kyvernoLoading, isRefreshing: kyvernoRefreshing, isDemoData: kyvernoDemoData, installed: kyvernoInstalled, hasErrors: kyvernoHasErrors, clustersChecked: kyvernoChecked, totalClusters: kyvernoTotal, unavailableReason: kyvernoUnavailable } = useKyverno()
  const { selectedClusters } = useGlobalFilters()
  const { startMission } = useMissions()
  const [showBreakdown, setShowBreakdown] = useState(false)
  const isLoading = kubescapeLoading || kyvernoLoading
  const totalChecking = Math.max(kubescapeTotal, kyvernoTotal)
  const minChecked = Math.min(kubescapeChecked, kyvernoChecked)
  const allChecked = minChecked >= totalChecking && totalChecking > 0

  const { score, breakdown, usingFallback } = useMemo(
    () => buildComplianceScoreSummary({ kubescapeStatuses, kyvernoStatuses, selectedClusters }),
  ...
  const kyvernoBreakdownData = useMemo(() => {
    ...
    for (const [clusterName, status] of Object.entries(kyvernoStatuses)) {
      ...
      if (selectedClusters.length > 0 && !selectedClusters.includes(clusterName)) {
        continue
      }
  ...
  const isDemoData = kubescapeDemoData || kyvernoDemoData
  const hasData = !usingFallback || isDemoData
  const isFailed = kubescapeHasErrors && kyvernoHasErrors
  useCardLoadingState({
    isLoading: isLoading && !hasData,
    isRefreshing: kubescapeRefreshing || kyvernoRefreshing,
    hasAnyData: hasData,
    isDemoData,
    isFailed,
  })
  ...
  const noToolsInstalled = !isLoading && !kubescapeInstalled && !kyvernoInstalled && !isDemoData
  ...
  if (kubescapeUnavailable && kyvernoUnavailable) {
    return (
      ...
      {!noToolsInstalled && (
        <>
          ...
          <ComplianceScoreBreakdownModal
            ...
            score={score}
            breakdown={breakdown}
            kubescapeData={kubescapeAggregated.totalControls > 0 ? {
              totalControls: kubescapeAggregated.totalControls,
              passedControls: kubescapeAggregated.passedControls,
              failedControls: kubescapeAggregated.failedControls,
              frameworks: kubescapeAggregated.frameworks || [],
            } : undefined}
