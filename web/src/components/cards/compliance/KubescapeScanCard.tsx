...
export function KubescapeScan({ config: _config }: CardConfig) {
  ...
  const installedClusters = useMemo(() => Object.keys(statuses).filter((cluster) => statuses[cluster].installed), [statuses])
        ...
        {(() => {
          ...
          return (
            <div className="text-center mb-2">
              ...
              <p className="text-[10px] text-muted-foreground mt-0.5 leading-tight">{scoreContext.description}</p>
            </div>
          )
        })()}
