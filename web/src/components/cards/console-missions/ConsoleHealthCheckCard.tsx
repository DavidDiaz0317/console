...
export function ConsoleHealthCheckCard(_props: ConsoleMissionCardProps) {
  ...
  const { drillToAllClusters, drillToPod } = useDrillDownActions()
...
6. Security posture assessment`,
  ...
  // Calculate health score (0-100) from cluster health only so the horseshoe
  // value matches the healthy/unhealthy/offline cluster breakdown shown below.
  const healthScore = Math.round(clusters.length > 0
    ...
    <div className="h-full flex flex-col relative">
      ...
      <div className="grid grid-cols-2 @md:grid-cols-3 gap-2 mb-2 text-center">
        ...
        <div
          ...
          onClick={() => {
            if (unreachableClusters > 0) drillToAllClusters('unreachable')
          }}
