...
const PROVIDER_COLORS: Record<string, string> = {
  ...
  Ollama: 'bg-secondary text-muted-foreground border-gray-500/20',
  AzureOpenAI: 'bg-purple-500/10 text-purple-400 border-purple-500/20',
}

function ProviderBadge({ provider }: { provider: string }) {
  const classes = PROVIDER_COLORS[provider] || 'bg-secondary text-muted-foreground border-gray-500/20'
  return (
    <span className={`inline-flex items-center px-1.5 py-0.5 text-2xs font-medium rounded border ${classes}`}>
      {provider}
    </span>
