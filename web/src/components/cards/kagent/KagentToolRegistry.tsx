...
function ProtocolBadge({ protocol }: { protocol: string }) {
  const colorMap: Record<string, string> = {
    stdio: 'bg-secondary text-muted-foreground border-gray-500/20',
  ...
  const classes = colorMap[protocol] || 'bg-secondary text-muted-foreground border-gray-500/20'
  return (
    <span className={`inline-flex items-center px-1.5 py-0.5 text-2xs font-medium rounded border ${classes}`}>
      {protocol}
    </span>
              ...
              {isExpanded && toolCount > 0 && (
                <div className="ml-8 mr-2 mb-1 space-y-0.5">
                  {tool.discoveredTools.map(dt => (
                    <div key={dt.name} className="text-xs text-muted-foreground flex items-center gap-1.5 px-2 py-0.5 rounded bg-secondary/60">
