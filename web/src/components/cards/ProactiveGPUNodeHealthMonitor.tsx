...
function CronJobClusterPanel({ cluster }: { cluster: string }) {
  ...
  if (isLoading && !status) {
    return (
      <div className="flex items-center gap-2 px-3 py-2 text-xs text-muted-foreground/70">
    ...
    <div className="rounded-lg border border-border bg-secondary overflow-hidden">
      <div className="flex items-center gap-2 px-3 py-2">
        ...
        <div className="flex-1 min-w-0">
          {status?.installed ? (
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {status.version > 0 && (
                <span className="text-2xs text-muted-foreground/50">v{status.version}</span>
              )}
            </div>
          ) : (
            <div className="flex items-center gap-2">
              <XCircle className="w-3.5 h-3.5 text-muted-foreground/60 shrink-0" />
              <span className="text-xs text-muted-foreground/70">{t('cards:gpuNodeHealth.notInstalled')}</span>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-1 shrink-0">
          {status?.installed ? (
            <>
              ...
              {status.lastResults && status.lastResults.length > 0 && (
                <button
                  ...
                  className={cn(
                    ...
                    showResults ? 'bg-blue-500/15 text-blue-400' : 'text-muted-foreground/60 hover:text-muted-foreground'
                  )}
              ...
              {!showConfirmUninstall ? (
                <button
                  ...
                  className="p-1 rounded hover:bg-red-500/10 text-muted-foreground/60 hover:text-red-400 transition-colors disabled:opacity-50"
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button
                    ...
                    className="px-2 py-0.5 text-2xs rounded bg-secondary text-muted-foreground/70 hover:text-muted-foreground transition-colors"
                  >
            ...
            <span className="text-2xs text-muted-foreground/60 italic">{t('cards:gpuNodeHealth.noPermissions')}</span>
          )}
        </div>
      </div>
      ...
      {showInstallDialog && (
        <div className="border-t border-border px-3 py-2 bg-foreground/1 space-y-2">
          <div className="text-2xs text-muted-foreground uppercase tracking-wider">{t('cards:gpuNodeHealth.installCronJob')}</div>
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-2xs text-muted-foreground/70 block mb-0.5">{t('cards:gpuNodeHealth.namespace')}</label>
              <input
                type="text"
                value={namespace}
                onChange={e => setNamespace(e.target.value)}
                className="w-full px-2 py-1 text-xs rounded border border-border bg-secondary text-foreground focus:outline-hidden focus:border-border"
              />
            </div>
            <div>
              <label className="text-2xs text-muted-foreground/70 block mb-0.5">{t('cards:gpuNodeHealth.scheduleCron')}</label>
              <input
                type="text"
                value={schedule}
          ...
          {/* Tier selector */}
          <div>
            <label className="text-2xs text-muted-foreground/70 block mb-0.5">{t('cards:gpuNodeHealth.checkTier')}</label>
            <select
              value={tier}
          ...
          <div className="flex items-center gap-2 justify-end">
            <button
              ...
              className="px-2 py-1 text-2xs rounded bg-secondary text-muted-foreground/70 hover:text-muted-foreground transition-colors"
            >
              {t('cards:gpuNodeHealth.cancel')}
            </button>
            <button
      ...
      {status?.installed && status.canInstall && (
        <div className="border-t border-border px-3 py-1.5 flex items-center gap-2">
          <span className="text-2xs text-muted-foreground/70">{t('cards:gpuNodeHealth.tierLabel')}</span>
          <select
            value={tier}
            onChange={e => setTier(Number(e.target.value))}
            className="px-1.5 py-0.5 text-2xs rounded border border-border bg-secondary text-muted-foreground focus:outline-hidden focus:border-border"
          >
            {TIER_OPTIONS.map(t => (
              <option key={t.value} value={t.value}>{t.label}</option>
            ))}
      ...
      {status?.installed && (status.activeJobs > 0 || status.failedJobs > 0 || status.successJobs > 0) && (
        <div className="border-t border-border px-3 py-1.5 flex items-center gap-3 text-2xs">
          <span className="text-muted-foreground/70">{t('cards:gpuNodeHealth.jobs')}</span>
          ...
          {status.lastRun && (
            <span className="text-muted-foreground/60 ml-auto">Last: {new Date(status.lastRun).toLocaleTimeString()}</span>
          )}
        </div>
      )}
      ...
      {showResults && status?.lastResults && status.lastResults.length > 0 && (
        <div className="border-t border-border px-3 py-2 bg-foreground/1 space-y-1.5">
          <div className="text-2xs text-muted-foreground uppercase tracking-wider">{t('cards:gpuNodeHealth.latestResults')}</div>
          {status.lastResults.map(result => (
            <div key={result.nodeName} className="rounded border border-border bg-secondary p-2">
              <div className="flex items-center gap-2 mb-1">
                <span className="text-xs font-mono text-foreground">{result.nodeName}</span>
                <StatusBadge status={result.status} />
                {result.gpuCount != null && (
                  <span className="text-2xs text-muted-foreground/60">{result.gpuCount} GPUs</span>
                )}
              </div>
      ...
      {error && (
        <div className="border-t border-border px-3 py-1.5 text-xs text-red-400/80 flex items-center gap-1.5">
          ...
          {error}
          <button onClick={refetch} className="ml-auto text-2xs text-muted-foreground/70 hover:text-muted-foreground underline">
    ...
    <div className="flex flex-col gap-2 h-full">
      ...
      <div className="flex gap-2">
        ...
        <div className={cn('flex-1 rounded-lg px-3 py-2 text-center', summary.unhealthy > 0 ? 'bg-red-500/10 ring-1 ring-red-500/20' : 'bg-secondary')}>
          <div className={cn('text-lg font-bold', summary.unhealthy > 0 ? 'text-red-400' : 'text-muted-foreground/60')}>{summary.unhealthy}</div>
          <div className="text-2xs text-muted-foreground/70 uppercase tracking-wider">{t('cards:gpuNodeHealth.unhealthy')}</div>
        </div>
        <div className={cn('flex-1 rounded-lg px-3 py-2 text-center', summary.degraded > 0 ? 'bg-yellow-500/10 ring-1 ring-yellow-500/20' : 'bg-secondary')}>
          <div className={cn('text-lg font-bold', summary.degraded > 0 ? 'text-yellow-400' : 'text-muted-foreground/60')}>{summary.degraded}</div>
          <div className="text-2xs text-muted-foreground/70 uppercase tracking-wider">{t('cards:gpuNodeHealth.degraded')}</div>
        </div>
        <div className={cn('flex-1 rounded-lg px-3 py-2 text-center', summary.healthy > 0 ? 'bg-green-500/10' : 'bg-secondary')}>
          <div className={cn('text-lg font-bold', summary.healthy > 0 ? 'text-green-400' : 'text-muted-foreground/60')}>{summary.healthy}</div>
          <div className="text-2xs text-muted-foreground/70 uppercase tracking-wider">{t('cards:gpuNodeHealth.healthy')}</div>
        </div>
      </div>
        ...
        extra={
          <div className="flex items-center gap-1">
            {search && (
              <button
                onClick={() => setSearch('')}
                className="px-2 py-1 text-xs rounded border border-border bg-secondary text-muted-foreground hover:text-white/70"
              >
            ...
            <button
              ...
              className={cn(
                'p-1 rounded transition-colors',
                showCronJobPanel ? 'bg-blue-500/15 text-blue-400' : 'text-muted-foreground/60 hover:text-muted-foreground hover:bg-secondary'
              )}
      ...
      <div className="relative">
        <input
          ...
          className="w-full px-3 py-1.5 text-xs rounded border border-border bg-secondary text-foreground placeholder:text-muted-foreground/60 focus:outline-hidden focus:border-border"
        />
      </div>
      ...
      {showCronJobPanel && (
        <div className="rounded-lg border border-blue-500/20 bg-blue-500/3 p-2 space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-2xs text-muted-foreground/60">{t('cards:gpuNodeHealth.automatedChecks')}</span>
          </div>
            ...
            <div key={nodeKey} className="rounded-lg border border-border bg-secondary overflow-hidden">
              ...
              {isExpanded && (
                <div className="border-t border-border px-4 py-2 bg-foreground/1">
                  {/* GPU type */}
                  <div className="text-xs text-muted-foreground mb-2">{node.gpuType}</div>
                  ...
                  {(node.issues || []).length > 0 && (
                    <div className="mt-2 pt-2 border-t border-border">
                      <div className="text-2xs text-muted-foreground/70 uppercase tracking-wider mb-1">{t('cards:gpuNodeHealth.issues')}</div>
