...
export function MissionBrowserFilterPanel({
  ...
  return (
    <div
      id="mission-browser-filters"
      data-testid="mission-browser-filters"
      className="px-4 py-3 bg-card border-b border-border space-y-3 max-h-[40vh] md:max-h-[50vh] overflow-y-auto"
    >
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
        {activeFilterCount > 0 && (
          <button
            ...
            className="inline-flex w-fit items-center gap-1 px-2 py-1 text-[11px] rounded-full bg-red-500/10 text-red-400 hover:bg-red-500/20 border border-red-500/20 transition-colors"
          >
            <X className="w-3 h-3" />
            Clear all
          </button>
          ...
          <div className="flex items-center gap-1 overflow-x-auto pb-1 sm:overflow-visible sm:pb-0">
            {MATCH_PCT_OPTIONS.map((pct) => (
              <button
                ...
                className={cn(
                  'shrink-0 px-2 py-0.5 text-[11px] rounded-full transition-colors tabular-nums',
                  minMatchPercent === pct
          ...
          <div className="flex items-center gap-1 overflow-x-auto pb-1 sm:overflow-visible sm:pb-0">
            {(
              ...
              <button
                ...
                className={cn(
                  'shrink-0 px-2 py-0.5 text-[11px] rounded-full transition-colors',
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
          ...
          <div className="flex items-center gap-1 overflow-x-auto pb-1 sm:overflow-visible sm:pb-0">
            {['All', ...Array.from(facetCounts.missionClass.keys())].map((cls) => (
              <button
                ...
                className={cn(
                  'shrink-0 px-2 py-0.5 text-[11px] rounded-full transition-colors capitalize',
        ...
        <label className="flex flex-col gap-1 sm:flex-row sm:items-center sm:gap-2">
          ...
          <input
            ...
            className="w-full sm:w-36 px-2 py-1 text-[11px] bg-secondary border border-border rounded-lg text-foreground placeholder:text-muted-foreground focus:outline-hidden focus:ring-1 focus:ring-purple-500/40"
          />
        </label>
      </div>
