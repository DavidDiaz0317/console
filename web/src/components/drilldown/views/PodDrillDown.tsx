...
export function PodDrillDown({ data }: { data: Record<string, unknown> }) {
  ...
  return (
    <div className="-m-6 flex flex-col">
      ...
      <div className="border-b border-border px-6"><div className="flex gap-1">{TABS.map(tab => { const Icon = tab.icon; return <button key={tab.id} onClick={() => setActiveTab(tab.id)} aria-label={`${tab.label} tab`} className={cn('flex items-center gap-2 border-b-2 px-4 py-2 text-sm font-medium transition-colors', activeTab === tab.id ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:border-border hover:text-foreground')}><Icon className="h-4 w-4" />{tab.label}</button> })}</div></div>
