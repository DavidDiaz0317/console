    ...
    <div style={{ background: 'var(--s-surface-2)', border: '1px solid rgba(227,179,65,0.4)', borderRadius: 'var(--s-r)', padding: '8px 10px', marginBottom: 4 }}>
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        <button
          ...
          disabled={busy}
          style={{ background: 'var(--s-success)', color: 'var(--s-bg)', border: 'none', borderRadius: 'var(--s-rs)', padding: '4px 12px', fontSize: 11, fontWeight: 700, cursor: busy ? 'default' : 'pointer', opacity: busy ? 0.6 : 1 }}
        >
          {busy ? '...' : 'Approve'}
        </button>
        <button
