          ...
          <div className="flex gap-2 flex-wrap">
            {CATEGORIES.map(cat => (
              <button
                ...
                className={`px-3 py-1 rounded text-xs capitalize ${categoryFilter === cat ? 'bg-blue-500 text-white' : 'bg-secondary text-muted-foreground'}`}
              ...
              <tbody>
                {filteredRequirements.map(r => (
                  <tr key={r.id} className="border-b border-border/50 hover:bg-secondary/50">
                    <td className="p-3 font-mono text-blue-500">{r.id}</td>
                    <td className="p-3 text-foreground">{r.name}</td>
                    <td className="p-3"><span className="px-2 py-0.5 rounded bg-secondary text-muted-foreground text-xs capitalize">{r.category}</span></td>
                    <td className="p-3">
      ...
      {activeTab === 'clusters' && (
        <div className="space-y-4">
          {clusters.map(cluster => (
            <div key={cluster.id} className="bg-card rounded-lg border border-border p-4">
            ...
            <div className="space-y-3">
              {clusters.map(c => (
                <div key={c.id} className="flex items-center gap-3">
                  <span className="w-40 text-sm text-muted-foreground truncate">{c.name}</span>
                  <div className="flex-1 h-2 bg-secondary rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full ${
