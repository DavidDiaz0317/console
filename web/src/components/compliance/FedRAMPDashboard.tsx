...
const milestoneStatusBadge = (status: string) => {
  switch (status) {
    ...
    default: return <span className="px-2 py-0.5 rounded text-xs bg-secondary text-muted-foreground">{status}</span>
  }
}
      ...
      {score && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <div className="bg-card rounded-lg p-4 border border-border">
            ...
            <p className="text-2xl font-bold text-foreground capitalize">{score.authorization_status}</p>
          </div>
          ...
          <div className="flex gap-2 flex-wrap">
            {['all', 'satisfied', 'partially_satisfied', 'planned'].map(s => (
              <button
                ...
                className={`px-3 py-1 rounded text-xs ${statusFilter === s ? 'bg-blue-500 text-white' : 'bg-secondary text-muted-foreground'}`}
              ...
              <tbody>
                {filteredControls.map(c => (
                  <tr key={c.id} className="border-b border-border/50 hover:bg-secondary/50">
                    <td className="p-3 font-mono text-blue-500">{c.id}</td>
                    <td className="p-3 text-foreground">{c.name}</td>
                    <td className="p-3"><span className="px-2 py-0.5 rounded bg-secondary text-muted-foreground text-xs">{c.family}</span></td>
                    <td className="p-3">
            ...
            <tbody>
              {poams.map(p => (
                <tr key={p.id} className="border-b border-border/50 hover:bg-secondary/50">
                  <td className="p-3 font-mono text-blue-500">{p.id}</td>
          ...
          <div className="bg-card rounded-lg border border-border p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-foreground capitalize">{score.authorization_status}</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">Overall Score</p>
