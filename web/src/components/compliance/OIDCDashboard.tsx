            ...
            <tbody>
              {providers.map(p => (
                <tr key={p.id} className="border-b border-border/50 hover:bg-muted/50">
                  ...
                  <td className="p-3 text-muted-foreground font-mono text-xs truncate max-w-[200px]">{p.issuer_url}</td>
                  <td className="p-3">
                    <span className="px-2 py-1 bg-muted rounded text-xs text-muted-foreground border border-border">{p.protocol}</span>
                  </td>
                  <td className="p-3 text-muted-foreground">{p.users_synced}</td>
                  <td className="p-3 text-muted-foreground">{p.groups_mapped}</td>
                  <td className="p-3 text-muted-foreground text-xs">{new Date(p.last_sync).toLocaleString()}</td>
                  <td className="p-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium border ${STATUS_STYLES[p.status] || 'bg-muted text-muted-foreground border-border'}`}>
            ...
            <tbody>
              {sessions.map(s => (
                <tr key={s.id} className="border-b border-border/50 hover:bg-muted/50">
                  ...
                  <td className="p-3 text-muted-foreground">{s.provider_name}</td>
                  ...
                  <td className="p-3 text-muted-foreground font-mono text-xs">{s.ip_address}</td>
                  <td className="p-3">
                    {s.active ? (
