...
const SIEMDashboard = memo(function SIEMDashboard() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-64">
      ...
      <span className="ml-3 text-muted-foreground">Loading SIEM data…</span>
    </div>
  )
              ...
              <div className="space-y-3">
                {(summary.top_sources || []).map(src => (
                  <div key={src.source} className="flex items-center gap-3">
                    <span className="w-32 text-sm text-muted-foreground truncate">{src.source}</span>
                    <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
      ...
      {activeTab === 'events' && (
        <div className="bg-card/50 rounded-lg border border-border overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {events.map(evt => (
                <tr key={evt.id} className="border-b border-border/50 hover:bg-muted/50">
                  <td className="p-3 text-muted-foreground text-xs">{new Date(evt.timestamp).toLocaleTimeString()}</td>
                  <td className="p-3">
                  ...
                  <td className="p-3 text-muted-foreground">{evt.category}</td>
                  <td className="p-3 text-foreground truncate max-w-xs">{evt.message}</td>
                  <td className="p-3 text-muted-foreground">{evt.cluster}</td>
                </tr>
              ))}
            </tbody>
          </table>
              ...
              <tbody>
                {alerts.map(alert => (
                  <tr key={alert.id} className="border-b border-border/50 hover:bg-muted/50">
                    ...
                    <td className="p-3 text-muted-foreground text-xs">{new Date(alert.triggered_at).toLocaleString()}</td>
