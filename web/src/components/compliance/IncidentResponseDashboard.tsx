...
const IncidentResponseDashboard = memo(function IncidentResponseDashboard() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-64">
      ...
      <span className="ml-3 text-muted-foreground">Loading incident data…</span>
    </div>
  )
                ...
                <tr key={inc.id} className="border-b border-border/50 hover:bg-muted/50">
                  ...
                  <td className="p-3">
                    <span className="flex items-center gap-1.5">
                      ...
                      <span className="text-muted-foreground capitalize">{inc.status}</span>
                    </span>
                  </td>
                  <td className="p-3 text-muted-foreground">{inc.assignee}</td>
                  <td className="p-3 text-foreground">L{inc.escalation_level}</td>
                  <td className="p-3 text-muted-foreground">{inc.cluster}</td>
                  <td className="p-3 text-muted-foreground text-xs">{new Date(inc.updated_at).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
                ...
                {Object.entries(metrics.by_severity).map(([sev, count]) => {
                  ...
                  return (
                    <div key={sev} className="flex items-center gap-3">
                      <span className={`w-16 text-sm capitalize ${SEVERITY_COLORS[sev] || 'text-muted-foreground'}`}>{sev}</span>
              ...
              <div className="space-y-3">
                {Object.entries(metrics.by_status).map(([st, count]) => (
                  <div key={st} className="flex items-center gap-3">
                    ...
                    <span className="w-24 text-sm text-muted-foreground capitalize">{st}</span>
                    <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
