...
export default function LicenseComplianceDashboard() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-64">
      ...
      <span className="ml-3 text-muted-foreground">Scanning license inventory…</span>
    </div>
  )
  ...
  if (error) return (
    <div className="p-6 text-center">
      ...
      <button onClick={fetchData} className="px-4 py-2 bg-primary hover:bg-primary/90 text-primary-foreground rounded-lg text-sm">Retry</button>
    </div>
  )
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-card/50 border border-border rounded-xl p-4">
            <div className="text-sm text-muted-foreground mb-1">Denied Licenses</div>
            <div className={`text-3xl font-bold ${summary.denied_packages > 0 ? 'text-red-400' : 'text-emerald-400'}`}>
              {summary.denied_packages}
            </div>
            <div className="text-xs text-muted-foreground mt-1">must be remediated</div>
          </div>
          <div className="bg-card/50 border border-border rounded-xl p-4">
            ...
            <div className="text-xs text-muted-foreground mt-1">require legal review</div>
          </div>
          <div className="bg-card/50 border border-border rounded-xl p-4">
            <div className="text-sm text-muted-foreground mb-1">Allowed</div>
            <div className="text-3xl font-bold text-emerald-400">{summary.allowed_packages}</div>
            <div className="text-xs text-muted-foreground mt-1">of {summary.total_packages} total</div>
          </div>
          <div className="bg-card/50 border border-border rounded-xl p-4">
            <div className="text-sm text-muted-foreground mb-1">Unique Licenses</div>
            <div className="text-3xl font-bold text-indigo-400">{summary.unique_licenses}</div>
            <div className="text-xs text-muted-foreground mt-1">{summary.workloads_scanned} workloads scanned</div>
          </div>
        </div>
      )}
        ...
        {(['violations', 'inventory', 'categories'] as const).map((tab) => (
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
              activeTab === tab ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground hover:bg-background/80'
            }`}
          >
            {tab === 'violations'
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-sm transition-colors ${
                      filterRisk === risk
                        ? RISK_STYLES[risk]
                        : 'border-border bg-background/80 text-muted-foreground hover:bg-muted/50'
                    }`}
                  >
          ...
          <div className="bg-card/50 border border-border rounded-xl overflow-hidden">
            {displayPackages.length === 0 ? (
              <div className="p-8 text-center">
                ...
                <p className="text-muted-foreground">No license violations detected.</p>
              </div>
            ) : (
              <table className="w-full text-sm">
                ...
                <tbody>
                  {displayPackages.map((pkg, i) => {
                    ...
                    return (
                      <tr key={i} className="border-b border-border/50 hover:bg-background/80">
                        <td className="p-3">
                        ...
                        <td className="p-3">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-muted-foreground text-xs">{pkg.license}</span>
                          </div>
                          <div className="text-[10px] text-muted-foreground">{pkg.spdx_id}</div>
                        </td>
                        <td className="p-3">
                          <div className="text-muted-foreground">{pkg.workload}</div>
              ...
              <div key={i} className={`rounded-xl border p-4 ${RISK_STYLES[cat.risk]}`}>
                ...
                <div className="flex flex-wrap gap-1">
                  {cat.examples.map((ex, j) => (
                    <span key={j} className="px-1.5 py-0.5 bg-background/80 border border-border rounded text-[10px] font-mono">
                      {ex}
                    </span>
                  ))}
                </div>
