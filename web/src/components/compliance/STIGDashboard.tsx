...
const severityBadge = (severity: string) => {
  switch (severity) {
    ...
    case 'CAT III': return <span className="px-2 py-0.5 rounded text-xs font-semibold bg-secondary text-muted-foreground border border-border">CAT III</span>
    default: return <span className="px-2 py-0.5 rounded text-xs bg-secondary text-muted-foreground">{severity}</span>
  }
}
...
const statusIcon = (status: string) => {
  switch (status) {
    ...
    case 'not_applicable': return <Clock className="w-4 h-4 text-muted-foreground" />
          ...
          <div className="flex gap-2 flex-wrap">
            {['all', 'CAT I', 'CAT II', 'CAT III'].map(sev => (
              <button
                ...
                className={`px-3 py-1 rounded text-xs ${severityFilter === sev ? 'bg-blue-500 text-white' : 'bg-secondary text-muted-foreground'}`}
              ...
              <tbody>
                {filteredFindings.map(f => (
                  <tr key={f.id} className="border-b border-border/50 hover:bg-secondary/50">
                    <td className="p-3 font-mono text-blue-500">{f.rule_id}</td>
            ...
            <tbody>
              {benchmarks.map(b => (
                <tr key={b.id} className="border-b border-border/50 hover:bg-secondary/50">
                  <td className="p-3 font-mono text-blue-500">{b.id}</td>
          ...
          <div className="bg-card rounded-lg border border-border p-6">
            ...
            <div className="space-y-3">
              {[
                ...
                { label: 'CAT III (Low)', count: summary.cat_iii_open, color: 'bg-secondary', textColor: 'text-muted-foreground' },
              ].map(item => (
