...
export const MemoizedMessage = memo(function MemoizedMessage({ msg, missionAgent, isFullScreen, fontSize, isLastAssistantMessage, missionStatus }: MessageProps) {
  ...
  const markdownComponents = useMemo(() => ({
    ...
    h1: ({ children }: { children?: React.ReactNode }) => (
      <h1 className="mt-6 mb-3 pt-3 border-t border-border/30 first:border-t-0 first:pt-0 first:mt-0 text-xl font-bold">
        {children}
      </h1>
    ),

    h2: ({ children }: { children?: React.ReactNode }) => (
      <h2 className="mt-6 mb-3 pt-3 border-t border-border/30 first:border-t-0 first:pt-0 first:mt-0 text-lg font-bold">
        {children}
      </h2>
    ),

    h3: ({ children }: { children?: React.ReactNode }) => (
      <h3 className="mt-5 mb-2 pt-2 border-t border-border/20 first:border-t-0 first:pt-0 first:mt-0 text-base font-semibold">
        {children}
      </h3>
    ),
    ...
    h5: ({ children }: { children?: React.ReactNode }) => (
      <h5 className="mt-4 mb-2 font-medium">
        {children}
      </h5>
    ),

    h6: ({ children }: { children?: React.ReactNode }) => (
      <h6 className="mt-3 mb-2 font-medium">
        {children}
      </h6>
    ),

    p: ({ children }: { children?: React.ReactNode }) => <div className="my-4 leading-relaxed">{children}</div>,
