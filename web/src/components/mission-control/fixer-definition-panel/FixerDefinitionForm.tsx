...
interface FixerDefinitionFormProps {
  ...
  latestSystemErrorContent: string
...
export function FixerDefinitionForm({
  ...
  planningMission,
  planningFailed,
  latestSystemErrorContent,
  latestAIContent,
  installedProjects,
}: FixerDefinitionFormProps) {
  const [showManualAdd, setShowManualAdd] = useState(false)
  const [manualName, setManualName] = useState('')
  const shortcutLabel = typeof navigator !== 'undefined' && navigator.platform.includes('Mac') ? '⌘' : 'Ctrl'
    ...
    <div className="flex-1 overflow-y-auto px-8 py-6 space-y-6">
      ...
      <div>
        ...
        <div className="relative mt-1">
          ...
          <div className="absolute right-2 bottom-2 flex items-center gap-1.5">
            <span className="text-[10px] text-muted-foreground/50">{shortcutLabel}+Enter</span>
            <Button
              variant="primary"
              size="sm"
              onClick={onSubmit}
      ...
      {planningFailed && !aiStreaming && (
        <AISuggestErrorBanner
          errorContent={latestSystemErrorContent}
          onRetry={onSubmit}
