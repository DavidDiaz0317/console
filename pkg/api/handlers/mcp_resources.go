...
func (h *MCPHandlers) GetGPUNodes(c *fiber.Ctx) error {
	...
	if h.k8sClient != nil {
		...
		if cluster == "" {
			...
			resp := fiber.Map{"nodes": allNodes, "source": "k8s"}
...
func (h *MCPHandlers) GetNVIDIAOperatorStatus(c *fiber.Ctx) error {
	...
	if h.k8sClient != nil {
		...
		if cluster == "" {
			...
			waitWithDeadline(&wg, clusterCancel, maxResponseDeadline)
			resp := fiber.Map{"operators": allStatus, "source": "k8s"}
			if len(clusterErrs) > 0 {
				resp["partial"] = true
				resp["clusterErrors"] = clusterErrs
			}
...
func (h *MCPHandlers) GetConfigMaps(c *fiber.Ctx) error {
	...
	if h.k8sClient != nil {
		if cluster == "" {
			...
			waitWithDeadline(&wg, clusterCancel, maxResponseDeadline)
			resp := fiber.Map{"configmaps": allConfigMaps, "source": "k8s"}
			if len(clusterErrs) > 0 {
				resp["partial"] = true
				resp["clusterErrors"] = clusterErrs
			}
...
func (h *MCPHandlers) GetSecrets(c *fiber.Ctx) error {
	...
	if h.k8sClient != nil {
		if cluster == "" {
			...
			waitWithDeadline(&wg, clusterCancel, maxResponseDeadline)
			resp := fiber.Map{"secrets": allSecrets, "source": "k8s"}
			if len(clusterErrs) > 0 {
				resp["partial"] = true
				resp["clusterErrors"] = clusterErrs
			}
...
func (h *MCPHandlers) GetServiceAccounts(c *fiber.Ctx) error {
	...
	if h.k8sClient != nil {
		if cluster == "" {
			...
			waitWithDeadline(&wg, clusterCancel, maxResponseDeadline)
			resp := fiber.Map{"serviceAccounts": allServiceAccounts, "source": "k8s"}
			if len(clusterErrs) > 0 {
				resp["partial"] = true
				resp["clusterErrors"] = clusterErrs
			}
...
func (h *MCPHandlers) GetPVCs(c *fiber.Ctx) error {
	...
	if h.k8sClient != nil {
		if cluster == "" {
			...
			waitWithDeadline(&wg, clusterCancel, maxResponseDeadline)
			resp := fiber.Map{"pvcs": allPVCs, "source": "k8s"}
			if len(clusterErrs) > 0 {
				resp["partial"] = true
				resp["clusterErrors"] = clusterErrs
			}
...
func (h *MCPHandlers) GetPVs(c *fiber.Ctx) error {
	...
	if h.k8sClient != nil {
		if cluster == "" {
			...
			waitWithDeadline(&wg, clusterCancel, maxResponseDeadline)
			resp := fiber.Map{"pvs": allPVs, "source": "k8s"}
			if len(clusterErrs) > 0 {
				resp["partial"] = true
				resp["clusterErrors"] = clusterErrs
			}
...
func (h *MCPHandlers) GetResourceQuotas(c *fiber.Ctx) error {
	...
	if h.k8sClient != nil {
		if cluster == "" {
			...
			waitWithDeadline(&wg, clusterCancel, maxResponseDeadline)
			resp := fiber.Map{"resourceQuotas": allQuotas, "source": "k8s"}
			if len(clusterErrs) > 0 {
				resp["partial"] = true
				resp["clusterErrors"] = clusterErrs
			}
...
func (h *MCPHandlers) GetLimitRanges(c *fiber.Ctx) error {
	...
	if h.k8sClient != nil {
		if cluster == "" {
			...
			waitWithDeadline(&wg, clusterCancel, maxResponseDeadline)
			resp := fiber.Map{"limitRanges": allRanges, "source": "k8s"}
			if len(clusterErrs) > 0 {
				resp["partial"] = true
				resp["clusterErrors"] = clusterErrs
			}
