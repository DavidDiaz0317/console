...
func ACMMScanHandler(c *fiber.Ctx) error {
	...
	if loaded {
		...
		case <-waiter.done:
			if waiter.err != nil {
				...
				return c.Status(fiber.StatusBadGateway).JSON(fiber.Map{"error": "ACMM scan failed"})
			}
	...
	if err != nil {
		...
		return c.Status(fiber.StatusBadGateway).JSON(fiber.Map{"error": "GitHub API request failed"})
	}
