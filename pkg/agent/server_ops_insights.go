...
func (s *Server) handleVClusterCheck(w http.ResponseWriter, r *http.Request) {
	...
	if context != "" {
		...
		if err != nil {
			slog.Warn("[vCluster] failed to check vcluster on cluster", "context", context, "error", err)
			http.Error(w, "failed to check vcluster status", http.StatusInternalServerError)
			return
		}
		writeJSON(w, status)
		return
	...
	if err != nil {
		slog.Warn("[vCluster] failed to check vcluster on all clusters", "error", err)
		http.Error(w, "failed to check vcluster status", http.StatusInternalServerError)
		return
	}
