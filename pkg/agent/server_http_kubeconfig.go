...
func (s *Server) handleRenameContextHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// SECURITY: Validate context names to prevent flag injection (#14238)
	if err := validateKubeContext(req.OldName); err != nil {
		w.WriteHeader(http.StatusBadRequest)
		writeJSON(w, protocol.ErrorPayload{Code: "invalid_context_name", Message: "Invalid old context name"})
		return
	}
	if err := validateKubeContext(req.NewName); err != nil {
		w.WriteHeader(http.StatusBadRequest)
		writeJSON(w, protocol.ErrorPayload{Code: "invalid_context_name", Message: "Invalid new context name"})
		return
	}
