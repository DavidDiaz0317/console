...
func (s *Server) handleCanIHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		slog.Warn("RBAC check failed", "cluster", req.Cluster, "verb", req.Verb, "resource", req.Resource, "error", err)
		writeJSONError(w, http.StatusInternalServerError, "RBAC check failed")
		return
	}
...
func (s *Server) handleClusterPermissionsHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if cluster != "" {
		...
		if err != nil {
			slog.Warn("failed to get cluster permissions", "cluster", cluster, "error", err)
			writeJSONError(w, http.StatusInternalServerError, "failed to get cluster permissions")
			return
		}
		writeJSON(w, perms)
		return
	...
	if err != nil {
		slog.Warn("failed to get cluster permissions", "error", err)
		writeJSONError(w, http.StatusInternalServerError, "failed to get cluster permissions")
		return
	}
	writeJSON(w, perms)
...
func (s *Server) handlePermissionsSummaryHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		slog.Warn("failed to get permissions summary", "error", err)
		writeJSONError(w, http.StatusInternalServerError, "failed to get permissions summary")
		return
	}
